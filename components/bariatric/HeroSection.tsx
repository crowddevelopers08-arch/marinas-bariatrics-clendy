  ...
  return (
    <section className="bg-[#e3f9f9] overflow-hidden py-10 md:py-14">
      <div className="w-[min(1200px,92%)] mx-auto">
        ...
        <div className="relative bg-white rounded-[28px] overflow-hidden border border-[rgba(66,200,200,0.22)]">

          {/* Decorative ring — top right */}
          <div
            className="absolute top-0 right-0 w-[360px] h-[360px] rounded-full pointer-events-none z-0"
          ...
          {/*
            Desktop: 2-col grid — left col has badge+title (row 1) and desc+btn (row 2), video spans both rows on right
            Mobile:  flex-col — DOM order gives: badge+title → video → desc+btn
          */}
          <div className="relative z-10 grid grid-cols-[1fr_52%] max-[768px]:flex max-[768px]:flex-col">

            {/* ── Badge + Title  (desktop col-1 row-1 | mobile order-1) ── */}
            ...
            {/* ── Video  (desktop col-2 row-1–2 | mobile order-2) ── */}
            ...
            {/* ── Description + Button  (desktop col-1 row-2 | mobile order-3) ── */}
